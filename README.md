...
* a leader-election-based group-membership service exists between running instances of the application to facilite internal caching of user information
...

{
    ...
    contain the claim and are encoded as base64.  In order to ensure the JWT content has not been
    ...
    that the correct claims were embeddd in the JWT header and payload.  This is a pauper's
    single-sign-on ;)

    "jwt_sign_method": "ES384",
    'jwt_sign_method' informs the application which key set should be used to sign the JWT created
    during the login process.  In this example, the ecdsa384_pub_key_file & ecdsa384_priv_key_file 
    will be used to sign the JWT.  Note that this only applies to JWTs that are created via a direct
    login into the application via the https://<appurl:port>/login end-point.  It is possible that
    a user will access the application using a JWT generated somewhere with a set of keys that are
    also known to the application.
