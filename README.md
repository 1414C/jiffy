...
Jiffy is a model-based RESTful application services generator written in Go.  It was developed as an experiment to offer an alternative avenue when developing native applications for SAP Hana, but works nicely with Postgres, MariaDB and SQLite as well.  The jiffy application allows a developer to treat the data persistence layer as an abstraction thereby allowing retargeting of the backend db system without the need to recompile.
